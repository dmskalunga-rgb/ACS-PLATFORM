...
const platformContextExpiryMigrationPath = resolve(
  'database/migrations/20260910000000_platform_context_expiry_projection.sql',
);
const platformContextExpiryRollbackPath = resolve(
  'database/rollbacks/20260910000000_platform_context_expiry_projection.sql',
);
...
const xcap005RolesPath = resolve('database/roles/xcap005_evidence_roles.sql');
const xcap005MigrationPath = resolve(
  'database/migrations/20260902000000_xcap005_evidence_chain_of_custody.sql',
);
const xcap005RollbackPath = resolve(
  'database/rollbacks/20260902000000_xcap005_evidence_chain_of_custody.sql',
);
const xcap005SeedPath = resolve('database/tests/fixtures/xcap005_evidence_seed.sql');
const xcap005TestPath = resolve(
  'database/tests/rls/xcap005_evidence_chain_of_custody_isolation.sql',
);
const xcap011RolesPath = resolve('database/roles/xcap011_fusion_roles.sql');
const xcap011MigrationPath = resolve(
  'database/migrations/20260909000000_xcap011_cognitive_fusion_m0.sql',
);
const xcap011RollbackPath = resolve(
  'database/rollbacks/20260909000000_xcap011_cognitive_fusion_m0.sql',
);
...
const aiGovM0bRolesPath = resolve('database/roles/aigov_m0b_ai_risk_governance_roles.sql');
const aiGovM0bMigrationPath = resolve(
  'database/migrations/20260913000000_aigov_m0b_ai_risk_governance.sql',
);
const aiGovM0bRollbackPath = resolve(
  'database/rollbacks/20260913000000_aigov_m0b_ai_risk_governance.sql',
);
const aiGovM0bTestPath = resolve('database/tests/rls/aigov_m0b_ai_risk_governance_isolation.sql');
...
const testRoles = [
  ...
  'acs_platform_mpa',
  'acs_xcap005_evidence_login_test',
...
try {
  await dropTestRoles();
  const aiGovM0bExists = await client.query(
    "SELECT to_regclass('ai_governance.risks') AS relation",
  );
  if (aiGovM0bExists.rows[0]?.relation !== null)
    await client.query(await readFile(aiGovM0bRollbackPath, 'utf8'));
  ...
  if (xcap011Exists.rows[0]?.relation !== null)
    await client.query(await readFile(xcap011RollbackPath, 'utf8'));
  const xcap005Exists = await client.query(
    "SELECT to_regclass('cyberdefense.evidence_records') AS relation",
  );
  if (xcap005Exists.rows[0]?.relation !== null)
  ...
  await client.query(await readFile(platformContextExpiryMigrationPath, 'utf8'));
  ...
  await client.query(await readFile(xcap005RolesPath, 'utf8'));
  ...
  await client.query(await readFile(aiGovM0bRolesPath, 'utf8'));
