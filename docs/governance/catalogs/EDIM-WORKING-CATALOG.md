...
| Customer Registry Web  | Customer Registry API  | `/api/v1/commercial/customers`; OIDC; tenant-scoped permissions      | Explicit loading/empty/error/unauthorized/forbidden/conflict | `IMPLEMENTED_PENDING_REMOTE_CI` |
| Customer Registry API  | PostgreSQL             | Trusted tenant grant, `commercial.customers`, FORCE RLS              | Transaction rollback and fail closed                         | `IMPLEMENTED_PENDING_REMOTE_CI` |
| Customer mutation      | Transactional outbox   | Versioned `commercial.customer.*` canonical envelope                 | Atomic rollback; contact PII excluded                        | `IMPLEMENTED_PENDING_REMOTE_CI` |
