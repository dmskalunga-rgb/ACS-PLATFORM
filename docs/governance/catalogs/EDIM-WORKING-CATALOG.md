...
| Plan Catalog API       | PostgreSQL             | Future trusted tenant grant, `commercial.plans`/features, FORCE RLS  | Fail closed; no tenant/body authority or financial fields    | `PRE_IMPLEMENTATION_GOVERNANCE_PACKAGE` |
| Plan Catalog mutation  | Transactional outbox   | Future versioned `commercial.plan.*` / `plan_feature.*` envelope     | Atomic rollback; minimal INTERNAL payload                    | `PRE_IMPLEMENTATION_GOVERNANCE_PACKAGE` |
