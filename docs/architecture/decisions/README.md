# ACS Architecture Decision Records

ADRs in this directory are implementation decisions subordinate to the ACS Enterprise v5.3
baseline. `ACCEPTED_ON_PHASE_0_BRANCH` means implemented for validation on the Phase 0 branch;
the decision becomes repository policy only after controlled review and merge.

| ADR      | Decision                                                    |
| -------- | ----------------------------------------------------------- |
| ADR-0001 | Repository architecture                                     |
| ADR-0002 | Frontend stack                                              |
| ADR-0003 | Backend stack                                               |
| ADR-0004 | Data architecture                                           |
| ADR-0005 | API architecture                                            |
| ADR-0006 | Event architecture                                          |
| ADR-0007 | Authentication and authorization boundary                   |
| ADR-0008 | AI Gateway boundary                                         |
| ADR-0009 | Observability architecture                                  |
| ADR-0010 | Deployment architecture                                     |
| ADR-0011 | Trusted identity and tenant context                         |
| ADR-0012 | Opaque transaction-bound tenant grants                      |
| ADR-0013 | Production OIDC/JWT authentication                          |
| ADR-0015 | Tenant-scoped Commercial Customer Registry                  |
| ADR-0016 | Tenant-scoped Commercial Lead Registry                      |
| ADR-0017 | Tenant-scoped Commercial Plan Catalog                       |
| ADR-0018 | Tenant-scoped Commercial Partner Registry                   |
| ADR-0019 | Tenant-scoped Commercial Opportunity Registry               |
| ADR-0020 | Tenant-scoped Commercial Proposal / Quotation (`PROPOSED`)  |
| ADR-0021 | Tenant-scoped Commercial Contract (`PROPOSED`)              |
| ADR-0022 | Tenant-scoped Commercial Subscription (`PROPOSED`)          |
| ADR-0023 | Tenant-scoped Commercial Entitlement (`PROPOSED`)           |
| ADR-0024 | Tenant-scoped Usage / Metering (`PROPOSED`)                 |
| ADR-0025 | Tenant-scoped Commercial Rating (`PROPOSED`)                |
| ADR-0026 | Canonical MPA consumption transaction boundary (`ACCEPTED`) |
| ADR-0027 | Cognitive Cyber Fusion M0 contract boundary (`ACCEPTED`)    |

## AI and security decision records

The ACS-XCAP-011 M0 decision set (`ADR-0027`, `AIDR-0001`, `SDR-0001`) is
`ACCEPTED_AND_CANONICALLY_INTEGRATED`; implementation authorization remains separate and runtime is
not implemented.

| Record    | Decision                                                                         |
| --------- | -------------------------------------------------------------------------------- |
| AIDR-0001 | Cognitive Cyber Fusion and canonical AI Core/Gateway boundary (`ACCEPTED`)       |
| SDR-0001  | Fusion tenant, authorization, evidence-trust and poisoning boundary (`ACCEPTED`) |
